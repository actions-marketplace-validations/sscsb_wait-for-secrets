...
[![Maintained by sscsb.github.io](https://img.shields.io/badge/maintained%20by-sscsb.github.io-blueviolet)](https://sscsb.github.io/?utm_source=github&utm_medium=organic_oss&utm_campaign=wait-for-secrets)
[![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/sscsb/wait-for-secrets/badge)](https://api.securityscorecards.dev/projects/github.com/sscsb/wait-for-secrets)
[![License: Apache 2.0](https://img.shields.io/badge/License-Apache%202.0-blue.svg)](https://raw.githubusercontent.com/sscsb/wait-for-secrets/main/LICENSE)
...
jobs:
  build:
    ...
    steps:
      ...
      - uses: sscsb/wait-for-secrets@v1
        id: get-otp
        with:
          secrets: |
            DUMMY_SECRET: 
...
1. Why does `wait-for-secrets` need `id-token: write` permission?
   ...
   It needs the `id-token: write` permission to authenticate to the SSCSB API. This is to ensure only the authorized workflow can retrieve the secrets.
...
2. How does `wait-for-secrets` work?
   ...
   2. The token is sent to the SSCSB API along with the needed list of secrets.
   3. SSCSB API authenticates the caller using the token and stores the list of secrets in a data store.
   ...
   6. The secrets are sent to the SSCSB API, where they are stored in the datastore.
   7. `wait-for-secrets` Action polls every 10 seconds to check if the secrets are available.
   8. If available, the SSCSB API returns the secret values to the Action.
   9. `wait-for-secrets` Action makes a call to the SSCSB API to clear the secrets in the datastore
